...
*
*******************************************************************************/
uint32_t ModbusUtils::Get_UART_Config(uint8_t ui8Databits, uint8_t ui8Parity, uint8_t ui8Stopbit){
...
uint8_t ModbusUtils::Modbus_ExpectedBytes_TCPIP(uint8_t aui8Buffer[]){
    uint8_t ui8Response=0;
    uint16_t ui16NumberOfRegister=0;
    uint8_t  ui8FunctionCode=0;
    ...
    if((ui8FunctionCode == MODBUS_FUNC_READ_COIL_REG) || (ui8FunctionCode == MODBUS_FUNC_READ_INPUT_STATUS_REG)){
        ...
        if(ui16NumberOfRegister < 8){
            ui8Response = MODBUS_TCPIP_MANDATORY_RESPONSE_BYTES;
        }else{
            ui8Response = ui16NumberOfRegister/8;
            ui8Response = MODBUS_TCPIP_MANDATORY_RESPONSE_BYTES + ui8Response + 1;
        }
    }else if((ui8FunctionCode == MODBUS_FUNC_READ_HOLDING_REG) || (ui8FunctionCode == MODBUS_FUNC_READ_INPUT_REG)){
        ui8Response = 3 + (ui16NumberOfRegister * 2);
    }
    else{
        ui8Response = 8;
    }
