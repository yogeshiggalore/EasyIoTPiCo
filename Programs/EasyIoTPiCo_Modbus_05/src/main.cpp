/*******************************************************************************
* Project Name: EasyIoT_22_05
*
* Version: 1.0
*
* Description:
* In this project esp32, modbus tcpip master protocol example
* Owner:
* Yogesh M Iggalore
*
********************************************************************************
...
IPAddress IP(192,168,43,108);
uint16_t  ui16Port = 502;
...
void ESPServer_ModbusUpdate(void){
    ...
    if(ui8FunCode < MODBUS_FUNC_WRITE_SINGLE_COIL){
        ...
        ui8MobdusResponse = ModTCPM.Exceute(IP,ui16Port,300,aui8ModbusTxBuf,ui8TxByteCount,&RxByteCount,&ResponseTimeout);
    ...
    sTxString = " ";
    for(ui8LoopCounter=0;ui8LoopCounter<(ui8TxByteCount+6);ui8LoopCounter++){
        sTxString = sTxString + String(ModTCPM.aui8ModbusTCPTxBuffer[ui8LoopCounter],HEX) + " ";
    }
    ...
    if(ui8MobdusResponse == MODBUS_RTU_ERR_NONE){
        doc["Error"] = 0;
        ...
        if((ui8FunCode == 0x03) | (ui8FunCode == 0x04)){
            ...
            ui16Value = ModUtils.aui8ModbusRxBuffer[11];
            ui16Value = ui16Value << 8;
            ui16Value = ui16Value | ModUtils.aui8ModbusRxBuffer[12];
            doc["reg2"] = ui16Value;

            ui16Value = ModUtils.aui8ModbusRxBuffer[13];
            ui16Value = ui16Value << 8;
            ui16Value = ui16Value | ModUtils.aui8ModbusRxBuffer[14];
            doc["reg3"] = ui16Value;

            ui16Value = ModUtils.aui8ModbusRxBuffer[15];
            ui16Value = ui16Value << 8;
            ui16Value = ui16Value | ModUtils.aui8ModbusRxBuffer[16];
            doc["reg4"] = ui16Value;
        ...
        if(ui8MobdusResponse == MODBUS_RTU_ERR_TIMEOUT){
            doc["Error"] = "timeout";
        }else if(ui8MobdusResponse == MODBUS_RTU_ERR_CRC){
            doc["Error"] = "error crc";
        }else if(ui8MobdusResponse == MODBUS_RTU_ERR_INVALID_FUNC){
            doc["Error"] = "error funcode";
        }else if(ui8MobdusResponse == MODBUS_RTU_ERR_INVALID_REG_ADDRESS){
            doc["Error"] = "error invalid addr";
        }else if(ui8MobdusResponse == MODBUS_RTU_ERR_INVALID_MODBUS_COMMAND){
