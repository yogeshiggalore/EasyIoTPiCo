/*******************************************************************************
* File Name: ModbusTCPSlave.cpp
*
* Version: 1.0
*
* Description:
* This is source file for modbus tcp server functions
*
* Owner:
* Yogesh M Iggalore
*
...
// Constructors ////////////////////////////////////////////////////////////////
ModbusTCPSlave::ModbusTCPSlave(void):MBServer(MODBUS_TCP_PORT),client(){}

void ModbusTCPSlave::Start(void){
    MBServer.begin();
    client = MBServer.available();
    Serial.println("Modbus TCPIP Server Started");
}

void ModbusTCPSlave::Update(void){
    Read();
    Process();
    Write();
}

void ModbusTCPSlave::Read(void){
    uint8_t  ui8Response;
    uint16_t ui16Result;
    ...
    if(ui8Response == 1){
        ...
        ui8LastFunCodeRequested = ModUtils.aui8ModbusRxBuffer[7];
        for(ui8LoopCounter=0;ui8LoopCounter<ui8Response;ui8LoopCounter++){
            Serial.print(ModUtils.aui8ModbusRxBuffer[ui8LoopCounter],HEX);
            Serial.print(" ");
...
void ModbusTCPSlave::Process(void){
    uint8_t  ui8Response;
    ...
    if(ui8Response == MODBUS_DATA_RECEIVED){
		...
		ui16TranscationId = ModUtils.aui8ModbusRxBuffer[0];
		ui16TranscationId = ui16TranscationId << 8;
		ui16TranscationId = ui16TranscationId | ModUtils.aui8ModbusRxBuffer[1];

		ui16ProtocolId = ModUtils.aui8ModbusRxBuffer[2];
		ui16ProtocolId = ui16ProtocolId << 8;
		ui16ProtocolId = ui16ProtocolId | ModUtils.aui8ModbusRxBuffer[3];

        if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_READ_COIL_REG){
        	Process_Coil_Status();
        }else if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_READ_INPUT_STATUS_REG){
            Process_Input_Status();
        }else if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_READ_HOLDING_REG){
            Process_Holding_Register();
        }else if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_READ_INPUT_REG){
            Process_Input_Register();
        }else if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_WRITE_SINGLE_COIL){
            Process_Force_Single_Coil();
        }else if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_WRITE_SINGLE_REG){   
            Process_Preset_Single_Register();
        }else if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_WRITE_MULTI_COIL){
            Process_Force_Multiple_Coil();
        }else if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_WRITE_MULTI_REG){
            ...
            ui16ModbusSlaveProcessResult = ui16ModbusSlaveProcessResult | 9;//number of bytes to send...
            Exception_Handling((0x80 | ModUtils.aui8ModbusRxBuffer[7]),0x01); // illegal function code
...
void ModbusTCPSlave::Write(void){
    ...
    if(ui8ByteCount > 0){// more than 0 means send response
            if(client.connected()){
            	client.write((const uint8_t *)ModUtils.aui8ModbusTxBuffer,ui8ByteCount);
        	}else{
            ...
            for(ui8LoopCounter=0;ui8LoopCounter<ui8ByteCount;ui8LoopCounter++){
                Serial.print(ModUtils.aui8ModbusTxBuffer[ui8LoopCounter],HEX);
                Serial.print(" ");
...
void ModbusTCPSlave::Exception_Handling(uint8_t ui8ErrorCode, uint8_t ui8ErrorValue){
    uint16_t ui16Result=0;
    uint8_t  ui8Value=0;
    
    memset(ModUtils.aui8ModbusTxBuffer, 0, MODBUS_TX_BUFFER_SIZE);
    ModUtils.aui8ModbusTxBuffer[0] = ui16TranscationId >> 8;
	ModUtils.aui8ModbusTxBuffer[1] = ui16TranscationId;
	ModUtils.aui8ModbusTxBuffer[2] = ui16ProtocolId >> 8;
	ModUtils.aui8ModbusTxBuffer[3] = ui16ProtocolId;
	ModUtils.aui8ModbusTxBuffer[4] = 0;
	ModUtils.aui8ModbusTxBuffer[5] = 3;
	ModUtils.aui8ModbusTxBuffer[6] = ui8TCPSlaveId;
    ModUtils.aui8ModbusTxBuffer[7] = ui8ErrorCode;
    ModUtils.aui8ModbusTxBuffer[8] = ui8ErrorValue;
}

void ModbusTCPSlave::Process_ErrorCode(uint8_t ui8FunctionError, uint8_t ui8ErrorCode){
...
*  
*******************************************************************************/
uint16_t ModbusTCPSlave::Is_DataReceived(void){
    ...
    if(ui16ModbusRxByteCounter > 6){
        if(bSlaveIdValidation){
            if(ModUtils.aui8ModbusRxBuffer[6] != ui8TCPSlaveId){
                ui16ModbusRxByteCounter = 0;
            }
        }

        if(ui16ModbusRxByteCounter > 7){
            if(ModUtils.Modbus_Verify_FunctionCode(ModUtils.aui8ModbusRxBuffer[7]) == 0){
                ui16ModbusRxByteCounter = 0;
            }
        }

        if(ui16ModbusRxByteCounter > 11){
            if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_WRITE_MULTI_COIL){
				if(ui16ModbusRxByteCounter > (ModUtils.aui8ModbusRxBuffer[12] + 12)){
					ui16Result = 1;
                	ui16Result = ui16Result << 8;   
                	ui16Result = ui16Result | ui16ModbusRxByteCounter;
                	ui16ModbusRxByteCounter = 0;
				}
            }else if(ModUtils.aui8ModbusRxBuffer[7] == MODBUS_FUNC_WRITE_MULTI_REG){
                if(ui16ModbusRxByteCounter > (ModUtils.aui8ModbusRxBuffer[12] + 12)){
...
*  
*******************************************************************************/
void ModbusTCPSlave::Process_Holding_Register(void){
    ...
    ui16StartRegister = ModUtils.aui8ModbusRxBuffer[8];
    ui16StartRegister = ui16StartRegister  << 8;
    ui16StartRegister = ui16StartRegister | ModUtils.aui8ModbusRxBuffer[9];
    
    ui16NumberofRegister = ModUtils.aui8ModbusRxBuffer[10];
    ui16NumberofRegister = ui16NumberofRegister  << 8;
    ui16NumberofRegister = ui16NumberofRegister |ModUtils.aui8ModbusRxBuffer[11];
    ...
    if((ui16NumberofRegister > MODBUSSLAVE_HOLDING_REG_SIZE) | (ui16NumberofRegister == 0)){
        ...
        if(ui16StartRegister > (MODBUSSLAVE_HOLDING_REG_SIZE-1)){
            ...
            if(ui16NumberofRegister > (MODBUSSLAVE_HOLDING_REG_SIZE - ui16StartRegister)){
    			...
    			ModUtils.aui8ModbusTxBuffer[ui8ArrayCounter++] = ui16TranscationId >> 8;
				ModUtils.aui8ModbusTxBuffer[ui8ArrayCounter++] = ui16TranscationId;
				ModUtils.aui8ModbusTxBuffer[ui8ArrayCounter++] = ui16ProtocolId >> 8;
				ModUtils.aui8ModbusTxBuffer[ui8ArrayCounter++] = ui16ProtocolId;
				ModUtils.aui8ModbusTxBuffer[ui8ArrayCounter++] = 0;
				ModUtils.aui8ModbusTxBuffer[ui8ArrayCounter++] = (ui16NumberofRegister * 2) + 3;
				ModUtils.aui8ModbusTxBuffer[ui8ArrayCounter++] = ui8TCPSlaveId;
...
void ModbusTCPSlave::Process_Input_Register(void){
    uint16_t ui16StartRegister=0;
    uint16_t ui16NumberofRegister=0;
    uint8_t  ui8LoopCounter=0;
    uint8_t  ui8ArrayCounter=0;
...
void ModbusTCPSlave::Process_Coil_Status(void){
    uint16_t ui16StartRegister=0;
    uint16_t ui16NumberofRegister=0;
    uint8_t  ui8ArrayCounter=0;
    uint16_t ui16Result=0;
...
*  
*******************************************************************************/
void ModbusTCPSlave::Process_Input_Status(void){
    uint16_t ui16StartRegister=0;
    uint16_t ui16NumberofRegister=0;
    uint8_t  ui8ArrayCounter=0;
    uint16_t ui16Result=0;
...
*  
*******************************************************************************/
void ModbusTCPSlave::Process_Force_Single_Coil(void){
    uint16_t ui16RegisterIndex=0;
    uint16_t ui16RegisterValue=0;
    uint8_t ui8LoopCounter=0;

    ui16RegisterIndex = ModUtils.aui8ModbusRxBuffer[8];
    ui16RegisterIndex = ui16RegisterIndex  << 8;
    ui16RegisterIndex = ui16RegisterIndex | ModUtils.aui8ModbusRxBuffer[9];
    
    ui16RegisterValue = ModUtils.aui8ModbusRxBuffer[10];
    ui16RegisterValue = ui16RegisterValue  << 8;
    ui16RegisterValue = ui16RegisterValue | ModUtils.aui8ModbusRxBuffer[11];
    ...
    if(ui16RegisterIndex > MODBUSSLAVE_COIL_STATUS_END_INDEX){
        ...
        if(ui16RegisterValue == 0xFF00){
            ...
            for(ui8LoopCounter=0;ui8LoopCounter<12;ui8LoopCounter++){
                ModUtils.aui8ModbusTxBuffer[ui8LoopCounter] = ModUtils.aui8ModbusRxBuffer[ui8LoopCounter];
            }
                    
            ui16ModbusSlaveProcessResult = MODBUS_RTU_ERR_NONE;
            ui16ModbusSlaveProcessResult = ui16ModbusSlaveProcessResult << 8;
            ui16ModbusSlaveProcessResult = ui16ModbusSlaveProcessResult | 12;//number of bytes to send...
...
*  
*******************************************************************************/
void ModbusTCPSlave::Process_Force_Multiple_Coil(void){
    uint16_t ui16RegisterIndex=0;
    ...
    ui16NumberOfBits = ModUtils.aui8ModbusRxBuffer[10];
    ui16NumberOfBits = ui16NumberOfBits  << 8;
    ui16NumberOfBits = ui16NumberOfBits | ModUtils.aui8ModbusRxBuffer[11];

    ui8Value = ModUtils.aui8ModbusRxBuffer[13];
...
*  
*******************************************************************************/
void ModbusTCPSlave::Process_Preset_Single_Register(void){
    uint16_t ui16RegisterIndex=0;
    uint16_t ui16RegisterValue=0;
    uint8_t ui8LoopCounter=0;
...
*  
*******************************************************************************/
void ModbusTCPSlave::Process_Preset_Multiple_Register(void){
    uint16_t ui16StartRegister=0;
    ...
    ui16RegisterCount = ModUtils.aui8ModbusRxBuffer[10];
    ui16RegisterCount = ui16RegisterCount << 8;
    ui16RegisterCount = ui16RegisterCount | ModUtils.aui8ModbusRxBuffer[11];
        ...
        if((ui16RegisterCount == 0) | (ui16RegisterCount > MODBUSSLAVE_HOLDING_REG_SIZE)){
            ...
            if((ui16StartRegister + ui16RegisterCount) > MODBUSSLAVE_HOLDING_REG_SIZE){
                ...
                for(ui8LoopCounter=ui16StartRegister;ui8LoopCounter<(ui16StartRegister + ui16RegisterCount);ui8LoopCounter++){
                    ui16Value = ModUtils.aui8ModbusRxBuffer[13+ui8IndexCounter++];
                    ui16Value = ui16Value << 8;
                    ui16Value = ui16Value | ModUtils.aui8ModbusRxBuffer[13+ui8IndexCounter++];
